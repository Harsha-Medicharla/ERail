...
<body>
            ...
            <div class="collapse navbar-collapse" id="navbarText">
                ...
                <span class="navbar-text" class="nav-item" style="margin-left: 10px;">
                    <a class="nav-link" href="index.php?logout='1'"
                        style="color: white; background: linear-gradient(to right, #007bff, #00a5ff); margin:10px;padding: 10px; border: none; border-radius: 10px;">Logout</a>
                </span>
            <?php endif ?>
            <?php if (!isset($_SESSION['username'])): ?>
                <div class="collapse navbar-collapse" id="navbarText">
    ...
    <div class="header"
        style="background: linear-gradient(to right, #007bff, #00a5ff); padding-top: 10px;margin-top:100px;">
        <h1 style="color:white;">BOOKING DETAILS <br><?php if (isset($_POST['pnr']))
            echo $_POST['pnr']; ?></h1>
    </div>
    <form action="pnr.php" method="post">
        ...
        <div class="input-group">
            <input type="text" name="pnr" value="<?php if (isset($_POST['pnr']))
                echo $_POST['pnr']; ?>" class="label">
        </div>
        <div class="input-group">
            <button type="submit" name="submit" class="btn"
                style="background: linear-gradient(to right, #007bff, #00a5ff); margin: 10px; padding: 10px; border: none; border-radius: 10px;">Submit</button><br><br>
        </div><br><br><br><br>
    </form>
    <?php
    $conn = mysqli_connect('localhost', 'root', '', 'registration');
    if (isset($_POST["submit"])) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // Store the class directly from the table
            $class = $row['Class'];
        }
        echo '</table>';

        echo "<br>";
                                                                                                                                                                                                                                    $seat_number = 53;  // Randomly generate seat number
        $booking_status = 'CONFIRMED';  // Status is confirmed for the most recent booking
    
        echo '<h1 style="margin-top: 40px; color:black;"><b>PASSENGER DETAILS</b></h1>';
        echo '<table class="container" style="margin-top: 0px;"> 
                <th style="background: linear-gradient(to right, #007bff, #00a5ff);"> Seat Number </th>
